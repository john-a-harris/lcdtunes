...
def main():
    ...
    # Set up a Moode screen
    moode_screen = lcd.add_screen("Moode")
    moode_screen.set_heartbeat("off")
    moode_title = moode_screen.add_title_widget("moode_title", text = "Moode")
    moode_screen.set_priority("hidden")
    moode_screen.set_duration(10)
    ...
    # Add fields to the Moode screen
    moode_line1 = moode_screen.add_scroller_widget("Moode_Line1", top = 2, direction = "m",  speed=3, text = "")
    moode_line2 = moode_screen.add_scroller_widget("Moode_Line2", top = 3, direction = "m",  speed=3, text = "")
    moode_line3 = moode_screen.add_scroller_widget("Moode_Line3", top = 4, direction = "m",  speed=3, text = "")
    ...
    with fifo as f:
        while True:
            ...
            if line.endswith("</item>"):
                ...
                if type == "mood":
                    # Check for Moode codes (note this is not part of the shairport metadata standard)
                    if code == "play":
                        logger.info("Moode playback started")
                        screen1.set_backlight("on")
                        moode_screen.set_priority("foreground")
                        screen1.set_priority("hidden")
                        spot_screen.set_priority("hidden")
                        moode_line2.set_text("Moode is playing... ")
                    if code == "stop":
                        logger.info("Moode playback stopped")
                        moode_screen.clear()
                        moode_screen.set_priority("hidden")
                        screen1.set_priority("info")
                        screen1.set_backlight("off")
                    if code == "mtit":
                        data=data.decode()
                        logger.info("Moode track: " + data)
                        moode_line1.set_text(pad_string(data))
                    if code == "malb":
                        data=data.decode()
                        logger.info("Moode album: " + data)
                        moode_line3.set_text(pad_string(data))
                    if code == "mart":
                        data=data.decode()
                        logger.info("Moode artist: " + data)
                        moode_line2.set_text(pad_string(data))

                if data != "":
                    #logger.info("Type: " + type + ", Code: " + code + ", Data: " + data.decode('utf-8'))
                    logger.info("Type: " + type + ", Code: " + code + ", Data: ")
                    logger.info(data)
                else:
